...
Tries Gemini if an API key is available; falls back to heuristic on any error.
...
from src.recommendations import gemini as _gemini
from src.recommendations.schema import is_valid

logger = logging.getLogger(__name__)

_FALLBACK_NOTE = (
    "⚠️ Gemini recommendations unavailable (offline mode or API error). "
...
def recommend(
    ...
    try:
        ...
        parsed = _gemini.parse_and_validate(raw)
        if parsed is not None and is_valid(parsed):
            return parsed, None
        logger.warning("service: gemini output invalid after repair; using heuristic")
    except Exception as exc:
        logger.warning("service: gemini call failed (%s); using heuristic", exc)
...
def recommend_stream(
    ...
    """Yield raw text chunks from Gemini streaming call.
    ...
    yield from _gemini.call_gemini_stream(
        snapshot=snapshot,
        api_key=resolved_key,
        model=model,
        temperature=temperature,
